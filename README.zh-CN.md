...
- `Package.swift` 当前依赖相邻目录 `../DesignKit`。如果直接构建，需要把 `DesignKit` 放在 `TimelineKit` 同级目录，或把少量依赖 DesignKit 的 UI 提示/控件替换成你自己的实现。
- 目前还没有测试 target。
...
当前本地仓库布局下，`Package.swift` 期待如下目录：
...
- 当前还没有完全清理成本可直接公开消费的 SPM 包，因为仍依赖本地 `../DesignKit`。
