...
struct SubtitleRenderFrame: @unchecked Sendable {
    let segmentID:       UUID
    let ciImage:         CIImage   // full-canvas transparent image with text drawn in
    let startTime:       Double    // seconds
    let endTime:         Double    // seconds
    let fadeInDuration:  Double    // enter fade (0 = instant)
    let fadeOutDuration: Double    // exit fade  (0 = instant)
}
