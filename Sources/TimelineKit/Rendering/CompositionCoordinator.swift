...
//#endif
    ...
    private func renderFrameAndFlush(force: Bool, retryCount: Int = 6) {
        ...
        guard let pixelBuffer = renderer.renderFrame(at: compositionTime) else {
#if DEBUG
            print("[TimelineRuntime] render nil force=\(force) retryCount=\(retryCount) t=\(formatDebugTime(cmTime)) storeIsPlaying=\(store?.isPlaying ?? false) playerRate=\(player.rate) providerBound=\(VideoLayerComposer.frameProvider === videoFrameProvider) preview=\(timelinePreviewView.debugStateDescription) provider=\(videoFrameProvider.debugStateDescription)")
#endif
